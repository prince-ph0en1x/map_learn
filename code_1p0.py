...
def drawMap():
	...
	imgfile = Image.open("images/WorldBWScaled.png")
	...
	scale = 2
	imgcx = imgfile.size[0]/scale
	imgcy = imgfile.size[1]/scale
	...
	epsimg.save("images/WorldSounak.png","png")
	win.close()
	...
	for ys in range (0,ymax):
		for xs in range (-xmax,xmax):
			...
			if (xc >= 0 and xc <= sz and yc >= 0 and yc <= sz): 
				...
				file.write(str(xs)+","+str(ys)+","+str(prob)+"\n")  # storing the hex cordinates text file to draw the map later
				#print(xs,ys,prob)
